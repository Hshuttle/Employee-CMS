...
function viewRoles() {
    db.promise()
      .query("SELECT roles.title AS Title, roles.salary AS Salary, department.department_name AS Department FROM roles LEFT JOIN department ON roles.department_id = department.id")
      .then(([response]) => {
          printTable(response)
          prompt()
      }
  ...
  function addDepartment(){
    inquirer.prompt({type: "input", name: "name", message: "What is the name of the new department"}).then(answers => {
        db.promise().query("INSERT INTO department SET ?", {department_name: answers.name}).then(([response]) => {
